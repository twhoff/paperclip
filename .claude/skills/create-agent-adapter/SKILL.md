...
**Before starting:** read `references/learning-log.md` in this skill's directory. It contains hard-won lessons from building real adapters — particularly the full list of UI registration gates that are not obvious from the codebase.
...
After creating the adapter package, register it in **all consumers and UI gates**. Missing any one of these causes the adapter to silently not appear or malfunction. See `references/learning-log.md` for the full story.

### 4.0 Shared Constants & Dependencies (do this first)

**`packages/shared/src/constants.ts`** — add the adapter type to `AGENT_ADAPTER_TYPES`:
...
```json
"@paperclipai/adapter-my-agent": "workspace:*"
```

Then run `pnpm install` to link the new package.
...
### 4.4 UI Visibility Gates (critical — easy to miss)
...
- Add to `effectiveAdapterCommand` ternary, debug command section, and auth hint section
...
- [ ] Added `"@paperclipai/adapter-<name>": "workspace:*"` to `cli/package.json`
...
- [ ] Adapter appears in New Agent dialog, onboarding wizard, and agent config dropdown
