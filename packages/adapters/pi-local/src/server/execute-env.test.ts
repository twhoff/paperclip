...
async function createEnvironmentReporter() {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-pi-env-"));
...
describe("Pi local child environment authority", () => {
  it("does not promote stale ambient Paperclip values into the trusted child env", async () => {
    ...
    const sessionPath = path.join(directory, "existing-session.jsonl");
    const context: AdapterExecutionContext = {
      ...
      runtime: {
        ...
        sessionParams: { sessionId: sessionPath, cwd: directory },
        sessionDisplayId: sessionPath,
        taskKey: null,
      },
      config: { command: executable, cwd: directory },
      context: {},
