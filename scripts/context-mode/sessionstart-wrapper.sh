...
PAPERCLIP_EXCEPTION='\\n  <paperclip_api_exception>\\n    <critical>Bash pcurl/curl to localhost/127.0.0.1 is ALLOWED and REQUIRED for Paperclip API calls.</critical>\\n    <reason>PAPERCLIP_* env vars (PAPERCLIP_API_KEY, PAPERCLIP_API_URL, etc.) exist ONLY in the Bash shell. They do NOT exist in context-mode sandboxes (ctx_execute, ctx_batch_execute).</reason>\\n    <rule>You MUST use the Bash tool with pcurl for ALL Paperclip API calls. Do NOT use ctx_execute or ctx_batch_execute for these. pcurl handles auth, URL resolution, and response compression automatically.</rule>\\n    <example>pcurl /api/agents/me</example>\\n  </paperclip_api_exception>'
