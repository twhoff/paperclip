...
| `model` | string | No | Codex model to use. Defaults to `gpt-5.6-sol` |
...
Legacy saved values such as `gpt-5`, `none`, or `minimal` remain readable for existing configs, but they are not curated choices and adapter switching will not introduce them.
